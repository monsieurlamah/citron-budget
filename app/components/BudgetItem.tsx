...
const BudgetItem: React.FC<BudgetItemProps> = ({ budget, enableHover }) => {
  const transactionCount = budget?.transactions
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        ...
        <div className="text-xl font-bold text-accent">{formatAmount(budget.amount)} GNF</div>
      </div>

      <div className="flex justify-between items-center mt-4 text-gray-500 text-sm">
        <span>{formatAmount(totalTransactionAmount)} GNF dépensés</span>
        <span>{formatAmount(remainingAmount)} GNF restants</span>
      </div>
